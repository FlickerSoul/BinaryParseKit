...
// MARK: - ExpressibleByRawBits Tests

@Suite("ExpressibleByRawBits Tests")
struct ExpressibleByRawBitsTests {
    @Test("UInt8 from RawBits with value 5")
    func uint8FromRawBits() throws {
    ...
    @Test("UInt8 from 4-bit RawBits")
    func uint8From4Bits() throws {
        // 0b1010 = 10
    ...
    func boolTrueFromRawBits() throws {
        let data = Data([0b1000_0000])
        let bits = RawBits(data: data, size: 1)

        let value = try Bool(bits: bits)
        #expect(value == true)
    }

    @Test("Bool from RawBits (false)")
    func boolFalseFromRawBits() throws {
        let data = Data([0b0000_0000])
        let bits = RawBits(data: data, size: 1)
    ...
    @Test("UInt16 from RawBits")
    func uint16FromRawBits() throws {
        // 0x1234 = 4660
        let data = Data([0x12, 0x34])
        let bits = RawBits(data: data, size: 16)

        let value = try UInt16(bits: bits)
        #expect(value == 0x1234)
    }
