...
public extension RawBits {
    ...
    func slice(from start: Int, count: Int) -> RawBits {
        ...
        let endByte = (start + count - 1) / Self.BitsPerWord

        // Slice the data
        let slicedData = data[(data.startIndex + startByte) ... (data.startIndex + endByte)]

        // Adjust the bit offset within the first byte
        let bitOffset = start % Self.BitsPerWord

        if bitOffset == 0 {
            // Aligned case - bits start at byte boundary
            return RawBits(data: Data(slicedData), size: count)
        } else {
            // Unaligned case - need to shift bits
            // For now, return the full bytes and adjust size
            // The caller should use extractBits for precise extraction
            return RawBits(data: Data(slicedData), size: min(count, slicedData.count * Self.BitsPerWord - bitOffset))
        }
    }
}
...
public extension RawBits {
    ...
    func sliceEquals(_ other: RawBits, at offset: Int = 0) -> Bool {
        ...
        for i in 0 ..< other.size {
            if bit(at: offset + i) != other.bit(at: i) {
                return false
            }
        }
        return true
